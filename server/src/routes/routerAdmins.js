...
router.get("/admin/:id/users", async (req, res) => {
  ...
  try {
    ...
    if (isAdmin.admin) {
      ...
      return res
        .status(404)
        .json({ error: "Lo sentimos no pudimos obtener los usuarios" });
    } else {
      res
        .status(404)
        .json({ error: "Lo sentimos Usted no es un ADMINISTRADOR" });
    }
  } catch (error) {
...
router.get("/admin/:id/events", async (req, res) => {
  ...
  try {
    ...
    if (isAdmin.admin) {
      ...
      return res
        .status(404)
        .json({ error: "Lo sentimos no pudimos obtener los eventos" });
    } else {
      res
        .status(404)
        .json({ error: "Lo sentimos Usted no es un ADMINISTRADOR" });
...
router.get("/admin/:id/reportuser", async (req, res) => {
  ...
  try {
    ...
    if (isAdmin.admin) {
      ...
      return res
        .status(404)
        .json({ error: "Lo sentimos no pudimos obtener los reportes" });
    } else {
      res
        .status(404)
        .json({ error: "Lo sentimos Usted no es un ADMINISTRADOR" });
...
router.get("/admin/:id/reviewuser", async (req, res) => {
  ...
  try {
    ...
    if (isAdmin.admin) {
      ...
      return res
        .status(404)
        .json({ error: "Lo sentimos no pudimos obtener los reviews" });
    } else {
      res
        .status(404)
        .json({ error: "Lo sentimos Usted no es un ADMINISTRADOR" });
